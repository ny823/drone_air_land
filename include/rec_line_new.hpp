...
void recbarcode(Mat& in_img, bool& isapp, int& j)
{
	...
	if (noline > 10)
	{
		...
		if (j > 0 && j < 10)
		{
			...
			cv::imwrite("/home/xy/acfly_ws/src/linetracing/img/" + std::to_string(j) + ".jpg", in_img);
			j++;
		}
...
Point recqrcode(Mat& in_img, bool& isapp, int& j)
{
	...
	if(center_all.size()>=3)
	{
		...
		{
			//std::cout << "paizao " << std::endl;
			cv::imwrite("/home/drone/acfly_ws/src/line-tracing/img/" + std::to_string(j)+"qrcode.jpg", in_img);
			j++;
		}
		return leftTop;
	}
